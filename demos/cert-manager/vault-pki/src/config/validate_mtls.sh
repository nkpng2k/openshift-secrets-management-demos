...
# Validates certificates issued via the Vault PKI -> Intermediate CA chain.

SERVER_URL="https://mtls-server.VAULT_ISSUER_NS.svc.cluster.local"
...
echo "=== Vault PKI mTLS Validation (in-pod) ==="
echo ""
...
if [[ "$RESPONSE" == *"Hello Vault PKI mTLS"* ]]; then
...
echo "--- Test 5: Server Certificate Chain ---"
echo | openssl s_client \
  -cert "$CLIENT_CERT" \
  -key "$CLIENT_KEY" \
  -CAfile "$CA_BUNDLE" \
  -connect mtls-server.VAULT_ISSUER_NS.svc.cluster.local:443 \
  -servername mtls-server.VAULT_ISSUER_NS.svc.cluster.local \
  2>/dev/null | openssl x509 -noout -issuer -subject -dates 2>/dev/null

echo ""

if [[ $FAILED -eq 0 ]]; then
  echo "=== All in-pod mTLS validations PASSED ==="
else
  echo "=== Some in-pod mTLS validations FAILED ==="
  exit 1
fi
